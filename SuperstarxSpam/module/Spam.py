...
from Superstarxspam import (HNDLR, SUDO_USERS, OWNER_ID, LOGS_CHANNEL)
...
async def delayspam(xspam: Client, e: Message): 
    ...
    if len(Superstarxspam) == 2:
       ...
       if re.search(Owners.lower(), msg.lower()):
            return await e.reply_text(usage)("**Sorry !!** I can't Spam On @sup3r_st4r_op's owner")
       sleeptime = float(Superstarxspam[0])
       if e.reply_to_message:
          reply_to_id = e.reply_to_message.message_id
          for _ in range(counts):
              await Superstarxspam.send_message(e.chat.id, msg, reply_to_message_id=reply_to_id)
              await asyncio.sleep(sleeptime)
          return
       for _ in range(counts):
           await Superstarxspam.send_message(e.chat.id, msg)
           await asyncio.sleep(sleeptime)
    else:
        await e.reply_text(usage)   
    if LOGS_CHANNEL:
         try:
             await Superstarxspam.send_message(LOGS_CHANNEL, f"started Delay Spam By User: {e.from_user.id} \n\n Chat: {e.chat.id} \n Counts: {counts} \n Spam Message: {msg} \n Delay Time: {sleeptime}")
...
async def pornspam(Superstarxspam: Client, e: Message): 
    counts = e.command[1]
    if not counts:
        return await e.reply_text(usage)
    if int(e.chat.id) in GROUP:
         return await e.reply_text("**Sorry !! i Can't Spam Here.**")
    Superstarxspam = "**#Pornspam**"
    count = int(counts)
    for _ in range(count):
         prn = choice(PORM)
         if ".jpg" in prn or ".png" in prn:
              await Superstarxspam.send_photo(e.chat.id, prn, caption=Superstarxspam)
              await asyncio.sleep(0.4)
         if ".mp4" in prn or ".MP4," in prn:
              await Superstarxspam.send_video(e.chat.id, prn, caption=Superstarxspam)
              await asyncio.sleep(0.4)
    if LOGS_CHANNEL:
         try:
            await Superstarxspam.send_message(LOGS_CHANNEL, f"started Porn Spam By User: {e.from_user.id} \n Chat: {e.chat.id} \n Counts: {count}")
         except Exception as a:
             print(a)
             pass
...
async def raid(xspam: Client, e: Message):  
      ...
      if len(Superstarxspam) == 2:
          ...
          if int(e.chat.id) in GROUP:
               return await e.reply_text("**Sorry !! i Can't Spam Here.**")
          ok = await Superstarxspam.get_users(Rizoel[1])
          id = ok.id
          if int(id) in Superstarxspam:
                text = f"I can't raid on @sup3r_st4r_op's Owner"
          ...
          if int(e.chat.id) in GROUP:
               return await e.reply_text("**Sorry !! i Can't Spam Here.**")
          #Superstarxspam = Superstarxspam.get_messages(e.chat.id, msg_id)
      ...
      if LOGS_CHANNEL:
         try:
            await Superstarxspam.send_message(LOGS_CHANNEL, f"started Raid By User: {e.from_user.id} \n\n On User: {mention} \n Chat: {e.chat.id} \n Counts: {counts}")
         except Exception as a:
             print(a)
             pass
...
async def fastspam(xspam: Client, e: Message):
    Superstarxspam = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 1)
    if len(Superstarxspam) == 2:
       counts = int(Superstarxspam[0])
       if int(e.chat.id) in GROUP:
            return await e.reply_text("**Sorry !! i Can't Spam Here.**")
       msg = str(Superstarxspam[1])
       if re.search(Owners.lower(), msg.lower()):
            return await e.reply("**Sorry !!** I can't Spam On @sup3r_st4r_op owner")
       if e.reply_to_message:
          reply_to_id = e.reply_to_message.message_id
          for _ in range(counts):
              await Superstarxspam.send_message(e.chat.id, msg, reply_to_message_id=reply_to_id)
    ...
    if LOGS_CHANNEL:
         try:
            await Superstarxspam.send_message(LOGS_CHANNEL, f"started Spam By User: {e.from_user.id} \n\n Chat: {e.chat.id} \n Counts: {counts} \n Spam Message: {msg}")
         except Exception as a:
             print(a)
             pass
