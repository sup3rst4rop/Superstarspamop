# SuperstarxSpam - Telegram Projects
# (c) 2022 - 2023 Star
# Don't Kang Bitch -!
...
from SuperstarxSpam import (OWNER_ID, HNDLR, SUDO_USERS, hl, LOGS_CHANNEL)
from pyrogram import Client, filters
from pyrogram.types import Message
from SuperstarxSpam.data import *

Usage = f"**❌ Wrong Usage ❌** \n Type: `{HNDLR}help owner`"
Media = "SuperstarxSpam/downloads/Profile.jpg"
...
async def setpic(SuperstarxSpam: Client, e: Message):
...
etc_bio = "USER OF SUPERSTAR X SPAM"
...
async def setname(SuperstarxSpam: Client, e: Message): 
      SuperstarxSpam = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(SuperstarxSpam) == 1:
          name = str(SuperstarxSpam[0])
          try:
            await SuperstarxSpam.update_profile(first_name=name, bio=etc_bio)
      ...
      if LOGS_CHANNEL:
         try:
             await SuperstarxSpam.send_message(LOGS_CHANNEL, f"Name Changed By User {e.from_user.id} \n\n New Name: {name}")
         except Exception as a:
             print(a)
             pass
...
async def setbio(xspam: Client, e: Message):
      SuperstarxSpam = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(SuperstarxSpam) == 1:
          xd = str(SuperstarxSpam[0])
          ok = await SuperstarxSpam.get_me()
          nam = ok.first_name
          nam2 = ok.last_name
          try:
             await SuperstarxSpam.update_profile(first_name=nam, last_name=nam2, bio=xd)
      ...
      if LOGS_CHANNEL:
         try:
             await SuperstarxSpam.send_message(LOGS_CHANNEL, f"Bio Changed By User: {e.from_user.id} \n\n New Bio: {xd}")
         except Exception as a:
             print(a)
             pass
