...
from SuperstarxSpam import OWNER_ID, HNDLR, DEVS, LOGS_CHANNEL
...
async def broadcast(xspam: Client, e: Message):
    ...
    async for cht in SuperstarxSpam.get_dialogs():
          try:
                await SuperstarxSpam.send_message(cht.chat.id, msg, disable_web_page_preview=True)
                dn += 1
                await asyncio.sleep(0.1)
          except Exception as e:
              err += 1 
    return await SuperstarxSpam.send_message(ok, f"**• Broadcast Done** ✅ \n\n Chats: {dn} \n Failed In {err} chats")
    if LOGS_CHANNEL:
       try:
           await SuperstarxSpam.send_message(LOGS_CHANNEL, f"Broadcasting Done By user {e.from_user.id} \n\n Chat: {dn} \n Failed in {err} chats")
